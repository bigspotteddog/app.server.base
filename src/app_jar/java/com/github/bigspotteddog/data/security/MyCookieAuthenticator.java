...
import com.github.bigspotteddog.data.security.model.Function;
...
public class MyCookieAuthenticator extends CookieAuthenticator {
    ...
    public MyCookieAuthenticator(final Context context, final boolean optional, final String realm,
    ...
    protected boolean authenticate(final Request request, final Response response) {
        ...
        final Class<? extends ServerResource> resourceClass = Utils
                .getTargetServerResource((WebSwitch) getApplication(), request);
    ...
    protected List<Role> getUserRoles(final Request request, final User user) {
        final List<Role> groups = (List<Role>) Utils.subrequest((WebSwitch) getApplication(),
                "roles?users.username=" + user.getIdentifier(), request);
        return groups;
    }
}

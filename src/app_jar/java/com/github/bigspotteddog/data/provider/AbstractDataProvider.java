...
public abstract class AbstractDataProvider<T> implements DataProvider<T>, AutoCreateVisitor {
    ...
    @SuppressWarnings("unchecked")
    protected EntityBase createOrUpdate(EntityBase target, boolean isFieldReadOnly, boolean isFieldCreate,
            boolean isFieldUpdate) throws Exception {
        if (target == null) {
            return target;
        }
        ...
        if (isNew(target) && create) {
            final DataProvider<EntityBase> dataProvider = (DataProvider<EntityBase>) DataProviderFactory.instance()
                    .getDataProvider(target.getClass());
    ...
    @SuppressWarnings("unchecked")
    protected void delete(final Object target, final boolean isEmbedded, final boolean isReadOnly,

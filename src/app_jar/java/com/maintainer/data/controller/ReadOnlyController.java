...
public abstract class ReadOnlyController extends ServerResource {
    ...
    public Representation handle() {
        ...
        if (text == null) {
            ...
            request2.getAttributes().put(ORG_RESTLET_HTTP_HEADERS, request.getAttributes().get(ORG_RESTLET_HTTP_HEADERS));
            ...
            if (isCaching() && entity != null && entity.getSize() != 0 && entity.getMediaType().equals(MediaType.APPLICATION_JSON)) {
                try {
                    text = entity.getText();
            ...
            synchronized(lock) {
                if (server == null) {
                    ...
                    if (isCaching()) {
                        ...
                        Cache<String, String> cache = CacheBuilder.newBuilder().expireAfterWrite(getCacheSeconds(), TimeUnit.SECONDS).build();
                        setCache(cache);
                    }
                }
            }

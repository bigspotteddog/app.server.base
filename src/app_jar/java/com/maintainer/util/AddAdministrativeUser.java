...
public class AddAdministrativeUser {
        ...
        if (list) {
            ...
            if (users.size() > 0) {
                if (reset) {
                    ...
                    log.debug("An administrative user was updated with username: " + admin.getUsername() + ", password: " + password + ".");
                } else {
                    log.debug("An administrative user with the username '" + username + "' already exists.");
                }
            } else {
                User admin = createUser(username, password);
                log.debug("An administrative user was added with username: " + admin.getUsername() + ", password: " + password + ".");
            }
        }
    }
    ...
    private User updateUser(User existing) throws Exception {
        DataProvider<User> users = (DataProvider<User>) DataProviderFactory.instance().getDataProvider(UserResourceController.getUserClass());
